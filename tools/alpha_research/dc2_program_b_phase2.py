"""Discovery Cycle 2 Program B Phase 2 governed campaign execution.

Program B Phase 2: Institutional Decision Ecology.
"""
...
def _with_reproducibility_hash(entity: dict[str, Any]) -> dict[str, Any]:
    ...
    if (
        ...
        or result["reproducibility_hash"] == "dc2-program-b-phase2-v1"
    ):
...
def _upsert_graph_payload(
    ...
    created_nodes = 0
    created_edges = 0
    model_node_id = "IKROS-PB2-WORLDMODEL-20260802-0001"
    conclusion_node_id = "IKROS-PB2-CONCLUSION-20260802-0001"
    evidence_node_id = "IKROS-PB2-EVIDENCE-20260802-0001"
    ...
    all_nodes = [
        GraphNode(
            ...
            label="Institutional Decision Ecology Model",
            confidence=0.74,
            spec_refs=["SPEC-060"],
            wp_refs=["WP-IMP-0048"],
            attributes={
                "program": "DC2 Program B Phase 2",
                "description": (
                    "Decision-layer model explaining how participant expectations "
                    "and belief updates create coordinated market actions."
                ),
            },
        ),
        GraphNode(
            ...
            label="Decision ecology explains coordinated market-wide behavior",
            confidence=0.72,
            spec_refs=["SPEC-060"],
            wp_refs=["WP-IMP-0048"],
            attributes={
                "arb_recommendation": analysis["institutional_recommendations"][
                    "arb_recommendation"
                ],
                "dominant_cascade_initiators": analysis[
                    "institutional_recommendations"
                ]["dominant_cascade_initiators"],
            },
        ),
        GraphNode(
            ...
            label="Program B Phase 2 Decision Ecology Evidence",
            confidence=0.70,
            spec_refs=["SPEC-060"],
            wp_refs=["WP-IMP-0048"],
            attributes={
                "evidence_ref": str(
                    DC2_PROGRAM_B_PHASE2_DIR / "DECISION_ECOLOGY_REPORT.md"
                )
            },
        ),
    ]
    all_nodes.extend(
        ...
        for node in payload["decision_nodes"] + payload["cascade_nodes"]
    )
    ...
    for edge in payload["belief_edges"]:
        add_edge(
            edge["source"],
            edge["target"],
            EdgeType.CAUSES.value,
            float(edge["confidence"]),
            str(DC2_PROGRAM_B_PHASE2_DIR / "BELIEF_UPDATE_REPORT.md"),
            dict(edge["attributes"]),
        )
    for edge in payload["strategic_edges"]:
        add_edge(
            edge["source"],
            edge["target"],
            EdgeType.DEPENDS_ON.value,
            float(edge["confidence"]),
            str(DC2_PROGRAM_B_PHASE2_DIR / "STRATEGIC_INTERACTION_MATRIX.md"),
            dict(edge["attributes"]),
        )
    add_edge(
        conclusion_node_id,
        ...
        0.74,
        str(DC2_PROGRAM_B_PHASE2_DIR / "INSTITUTIONAL_RECOMMENDATIONS.md"),
        {},
    )
    add_edge(
        conclusion_node_id,
        ...
        0.72,
        str(DC2_PROGRAM_B_PHASE2_DIR / "DECISION_ECOLOGY_REPORT.md"),
        {},
    )
    add_edge(
        model_node_id,
...
def run_dc2_program_b_phase2_campaign(repo_root: Path) -> dict[str, Any]:
    ...
    campaign = orchestrator.build_campaign(
        ...
        objective=(
            "Construct the first institutional decision ecology explaining how "
            "heterogeneous participant decisions generate market-wide behavior "
    ...
    analysis = prepare_dc2_program_b_phase2_artifacts(repo_root=repo_root)
    ...
    output_dir = repo_root / DC2_PROGRAM_B_PHASE2_DIR
    output_dir.mkdir(parents=True, exist_ok=True)
    report_paths = emit_dc2_program_b_phase2_reports(
        analysis,
        ...
        if not research_registry.exists(rq_id):
            try:
                rq_dict = {
                    ...
                    "lineage": {
                        "origin": {
                            "created_by": "dc2-program-b-phase2",
                            "created_at": "2026-08-02T00:00:00Z",
                            "creation_context": "Program B Phase 2 secondary question",
                    ...
                    "work_package_refs": [],
                    "version_history": [],
                    "title": f"DC2-PB2: {rq_secondary.get('theme', rq_id)}",
                ...
                _transition_if_needed(
                    ...
                    rq_id,
                    "ANSWERED",
                    note="Program B Phase 2 addressed this decision-ecology question.",
                )
            except Exception:
                pass
    ...
    _transition_if_needed(
        ...
        "ANSWERED",
        note="Program B Phase 2 completed: decision ecology model generated.",
    )
    ...
    metrics = {
        ...
        "n_participants": len(analysis["decision_profiles"]),
        "n_belief_edges": len(analysis["belief_update_network"]["edges"]),
        "n_strategic_edges": len(analysis["strategic_dependency_network"]["edges"]),
        "n_cascades": len(analysis["decision_cascade_models"]),
        "graph_nodes_created": graph_metrics["created_nodes"],
        "graph_edges_created": graph_metrics["created_edges"],
        "campaign_id": campaign.campaign_id,
        "reports": report_paths,
    }
    write_json(repo_root / "reports" / "dc2-program-b-phase2-metrics.json", metrics)

    return {
        "campaign_id": campaign.campaign_id,
        "campaign_result": report.to_dict(),
        "analysis_summary": analysis["institutional_recommendations"],
...
def _default_campaign_spec() -> dict[str, Any]:
    return {
        "title": "DC2 Program B Phase 2: Institutional Decision Ecology",
        "research_question_primary": {
            "ikros_id": "IKROS-RQ-20260802-5001",
            "entity_type": "ResearchQuestion",
            "version": "1.0.0",
            "lifecycle_state": "ANSWERED",
            "confidence": {
            ...
            "lineage": {
                "origin": {
                    "created_by": "dc2-program-b-phase2",
                    "created_at": "2026-08-02T00:00:00Z",
                    "creation_context": "Program B Phase 2 primary research question",
                    "motivation": (
                        "Program B Phase 1 established the ecology; "
                        "Phase 2 explains coordinated decision formation."
                    ),
                },
                "dependencies": {"supporting": [], "contradicting": [], "ers_records": []},
            },
            ...
            "version_history": [],
            "title": (
                "DC2-PB2: How do heterogeneous market participants "
                "transform new information into coordinated market decisions?"
            ),
            "motivation": (
                "Institutional understanding now requires a "
                "decision-formation layer above the approved participant ecology."
            ),
            "instrument": "XAU/USD",
            "scope": "CROSS_ASSET",
            "time_horizon": "1D",
            "reproducibility_hash": "dc2-program-b-phase2-rq-primary-v1",
        },
        "experiment": {
            "ikros_id": "IKROS-EXP-20260802-5001",
            "entity_type": "Experiment",
            "version": "1.0.0",
            ...
            "lineage": {
                "origin": {
                    "created_by": "dc2-program-b-phase2",
                    "created_at": "2026-08-02T00:00:00Z",
                    "creation_context": "Program B Phase 2 decision ecology experiment",
                    "motivation": (
                        "Model expectation formation, belief updating, "
                        "strategic dependence, and cascades."
                    ),
                },
                "dependencies": {"supporting": [], "contradicting": [], "ers_records": []},
            },
            ...
            "work_package_refs": [],
            "version_history": [],
            "title": "DC2-PB2-EXP-001: Institutional Decision Ecology",
            "description": (
                "Decision-layer model above participant ecology showing how "
                "market decisions coordinate into regime transitions."
            ),
            "reproducibility_hash": "dc2-program-b-phase2-exp-v1",
        },
        "research_questions_secondary": [
            {
                "ikros_id": "IKROS-RQ-20260802-5002",
                "theme": "Expectation Formation",
                "statement": (
                    "How do major participant classes form expectations "
                    "from macro, flow, and market-structure information?"
                ),
            },
            {
                "ikros_id": "IKROS-RQ-20260802-5003",
                "theme": "Belief Updating",
                "statement": (
                    "Which participants update beliefs fastest and how "
                    "do those updates propagate across the ecology?"
                ),
            },
            {
                "ikros_id": "IKROS-RQ-20260802-5004",
                "theme": "Strategic Interaction",
                "statement": (
                    "Which decision dependencies and strategic interactions "
                    "most strongly shape coordinated market behavior?"
                ),
            },
            {
                "ikros_id": "IKROS-RQ-20260802-5005",
                "theme": "Decision Cascades",
                "statement": (
                    "How do local participant decisions scale into "
                    "market-wide cascades and regime transitions?"
                ),
            },
        ],
    }
